...
\begin{align*}
	  \bm{\Gamma} = \left( \predDeltaU \right)^\T  \bm{Q} \predDeltaU  + \bm{R}  & \quad & \bm{\gamma}^\T  = 2\Tilde{\Bar{\bm{e}}}[k+1]^\T \bm{Q} \predDeltaU
\end{align*}
With $\Tilde{\Bar{\bm{e}}}[k+1] = \Bar{\bm{g}}[k] - \Bar{\bm{r}}[k+1]$ being the prediction of the quasi-tracking error. $\bm{Q}, \bm{R}$ must be chosen such that $\bm{\Gamma}  \succ 0$ is ensured.
