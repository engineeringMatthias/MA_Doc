...
\begin{figure}[H]
	...
	\label{fig:OaC_MPC}
\end{figure}
The MPC is provided with a reference prediction $\bar{\bm{r}}[k+1] = \left[ \bm{r}[k+1]\; \bm{r}[k+2] \; \hdots \bm{r}[k+N_P] \right]^\T$. Using current states and the previous input, the auxiliary prediction vector $\Bar{\bm{g} [k]}$ can be computed according to Equation \ref{equ:2_prediction}. Using $\Tilde{\Bar{\bm{e}}}[k] = \Bar{\bm{g}}[k] - \Bar{\bm{r}}[k+1]$ to calculate the linear term $\bm{\gamma}$ of the quadratic program, the cost function $J = \wLNorm{\Delta u}{\Gamma} + \bm{\gamma^T} \Deltaub$ is fully determined. To determine the constraints $\bm{M} \Deltaub \le \bm{\mu}$, the measured state vector and previous input are required. The output of the Quadratic Program \eqref{equ:2_QP} is the incremental input $\Deltaub$, which is accumulated then  to $\bm{u}[k]$. The interface to the plant is modeled as Sample and Hold elements. \\
...
A different approach is used by L. Wang \cite{mpcWang}. Here, the state space system is augmented, such that the states and the actuating signals are incremental. As it will be shown in Section \ref{sec:mpcc_analysis}, this ensures the MPC is of integrating behavior for locally unconstrained operation. \\
For discrete-time, linear time-invariant systems $\Sigma: \left[\bm{A}, \bm{B}, \bm{C} \right]$, the incremental states and inputs are defined as:
\begin{align*}
...
\begin{align*}
	\Sigma&: \left[\bm{A}(\omega[k]),\bm{B},\bm{E}(\omega[k])\right] - \Sigma:  \left[\bm{A}(\omega[k-1]),\bm{B},\bm{E}(\omega[k-1])\right] = \\
